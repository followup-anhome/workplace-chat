...
type T = { ja?: string; en?: string; detected?: string };
...
export default function Chat({ name, role, room, onBack }: {
  ...
  const parseT = (msg: Message): T => {
    try { return JSON.parse(msg.translation) as T; } catch { return {}; }
  };

  const getSenderFlag = (msg: Message) => {
    const t = parseT(msg);
    if (t.detected && DETECTED_FLAG[t.detected]) return DETECTED_FLAG[t.detected];
    return "💬";
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100dvh", maxWidth: "500px", margin: "0 auto", backgroundColor: "#f3f4f6" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "12px", display: "flex", flexDirection: "column", gap: "12px" }}>
        ...
        {messages.map(msg => {
          ...
          const t    = parseT(msg);
          const flag = getSenderFlag(msg);
          const hasJa = t.ja && t.ja.trim().length > 0;
          const hasEn = t.en && t.en.trim().length > 0;
            ...
            <div key={msg.id} style={{ display: "flex", flexDirection: "column", gap: "3px", maxWidth: "85%", alignSelf: isMe ? "flex-end" : "flex-start", alignItems: isMe ? "flex-end" : "flex-start" }}>
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: "5px", flexDirection: isMe ? "row-reverse" : "row" }}>
                <div style={{
                  ...
                  background: isMe ? "linear-gradient(135deg, #c0392b, #1a1a2e)" : "white",
              ...
              {/* 翻訳バブル - ja or en があれば必ず表示 */}
              {(hasJa || hasEn) && (
                <div style={{
                  fontSize: "11px", color: "#374151",
                  backgroundColor: "#e5e7eb",
                  borderRadius: "10px", padding: "6px 11px",
                  lineHeight: "1.8", wordBreak: "break-word", maxWidth: "100%",
                }}>
                  {hasJa && <div>🇯🇵 {t.ja}</div>}
                  {hasEn && <div>🇺🇸 {t.en}</div>}
                </div>
