...
export default function Chat({ name, role, room, onBack }: {
  ...
  const send = async () => {
    ...
    try {
      ...
      await supabase.from("messages").insert({
        ...
        original: text,
        translation: data.translations ? JSON.stringify(data.translations) : data.translated || "",
      });
  ...
  const parseT = (msg: Message): Translations => {
    try { return JSON.parse(msg.translation); } catch { return {}; }
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100dvh", maxWidth: "500px", margin: "0 auto", backgroundColor: "#f3f4f6" }}>
      ...
      {/* Header */}
      <div style={{ background: "linear-gradient(135deg, #1a1a2e, #c0392b)", padding: "11px 16px", display: "flex", alignItems: "center", gap: "12px", flexShrink: 0 }}>
        ...
        <div style={{ fontSize: "9px", background: "rgba(255,255,255,0.12)", borderRadius: "7px", padding: "3px 8px", color: "#fecaca", whiteSpace: "nowrap" }}>
          🇯🇵 🇺🇸 🇵🇭
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "12px", display: "flex", flexDirection: "column", gap: "12px" }}>
        {messages.length === 0 && (
          <div style={{ textAlign: "center", color: "#9ca3af", fontSize: "12px", marginTop: "30px" }}>
            ...
            <p>メッセージを送ってください</p>
            <p style={{ fontSize: "10px", marginTop: "4px" }}>🇯🇵 日本語 · 🇺🇸 English · 🇵🇭 Tagalog</p>
          </div>
        )}
        {messages.map(msg => {
          ...
          return (
            <div key={msg.id} style={{ display: "flex", flexDirection: "column", gap: "3px", maxWidth: "85%", alignSelf: isMe ? "flex-end" : "flex-start", alignItems: isMe ? "flex-end" : "flex-start" }}>
              <span style={{ fontSize: "10px", color: "#9ca3af", padding: "0 4px" }}>{flag} {msg.sender}</span>
              <div style={{ display: "flex", alignItems: "flex-start", gap: "5px", flexDirection: isMe ? "row-reverse" : "row" }}>
                <div style={{ padding: "9px 13px", borderRadius: isMe ? "16px 16px 4px 16px" : "16px 16px 16px 4px", fontSize: "14px", lineHeight: "1.55", wordBreak: "break-word", background: isMe ? "linear-gradient(135deg, #c0392b, #1a1a2e)" : "white", color: isMe ? "white" : "#111827", border: isMe ? "none" : "1px solid #e5e7eb" }}>
                  {msg.original}
                </div>
                {isMe && (
              ...
              {(t.ja || t.en) && (
                <div style={{ fontSize: "11px", color: "#374151", backgroundColor: "#e5e7eb", borderRadius: "10px", padding: "5px 10px", lineHeight: "1.7", wordBreak: "break-word", maxWidth: "100%" }}>
      ...
      <div style={{ backgroundColor: "white", borderTop: "1px solid #e5e7eb", padding: "10px 14px", flexShrink: 0 }}>
        <div style={{ display: "flex", gap: "8px", alignItems: "flex-end" }}>
          <textarea
            ...
            onKeyDown={handleKey}
            placeholder="日本語 / English / Tagalog..."
            rows={1}
          ...
          <button
            ...
            style={{ width: "42px", height: "42px", borderRadius: "50%", background: sending || !input.trim() ? "#9ca3af" : "linear-gradient(135deg, #c0392b, #1a1a2e)", border: "none", cursor: sending || !input.trim() ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}
          >
        ...
        <p style={{ fontSize: "8.5px", color: "#9ca3af", textAlign: "center", marginTop: "4px" }}>
          ファースト住建 Staff Chat · AI翻訳 by フォローアップ株式会社
        </p>
      </div>
    </div>
  );
