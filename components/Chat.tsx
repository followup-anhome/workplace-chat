...
export default function Chat({ name, langCode, room, onBack }: {
  ...
  const roomLabel = allRooms.find(r => r.id === room)?.label || `捷・・${room}`;
  ...
  const send = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setSendError(
          typeof data.error === "string" ? data.error : "鄙ｻ險ｳAPI繧ｨ繝ｩ繝ｼ (" + res.status + ")"
        );
        setInput(text);
        setSending(false);
        return;
      ...
      if (!data.translations || typeof data.translations !== "object") {
        setSendError("鄙ｻ險ｳ邨先棡繧貞叙蠕励〒縺阪∪縺帙ｓ縺ｧ縺励◆");
        setInput(text);
        setSending(false);
        return;
      }
      ...
      if (insErr) {
        setSendError(insErr.message || "繝｡繝・そ繝ｼ繧ｸ縺ｮ菫晏ｭ倥↓螟ｱ謨励＠縺ｾ縺励◆");
        setInput(text);
      }
    } catch (e) {
      console.error(e);
      setSendError("騾∽ｿ｡縺ｫ螟ｱ謨励＠縺ｾ縺励◆");
      setInput(text);
    }
    setSending(false);
  };
  ...
  const getSenderInfo = (msg: Message) => {
    ...
    return { flag: l?.flag || "町", label: l?.label || "" };
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100dvh", maxWidth: "500px", margin: "0 auto", backgroundColor: "#f3f4f6" }}>
      ...
      <div style={{ background: `linear-gradient(135deg, ${brand.dark}, ${brand.accent})`, padding: "11px 16px", display: "flex", alignItems: "center", gap: "12px", flexShrink: 0 }}>
        <button onClick={onBack} style={{ color: "white", fontSize: "20px", background: "none", border: "none", cursor: "pointer" }}>竊・/button>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ color: "#bfdbfe", fontSize: "10px" }}>
            ...
            {features.onlineCount && online > 0 && <span style={{ marginLeft: "7px", color: "#86efac" }}>笳・{online}莠ｺ</span>}
          </div>
        </div>
        <div style={{ fontSize: "9px", background: "rgba(255,255,255,0.12)", borderRadius: "7px", padding: "3px 8px", color: "#bfdbfe", whiteSpace: "nowrap" }}>{myLang?.flag} 縺ｧ陦ｨ遉ｺ荳ｭ</div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "12px", display: "flex", flexDirection: "column", gap: "12px" }}>
        {messages.length === 0 && (
          <div style={{ textAlign: "center", color: "#9ca3af", fontSize: "12px", marginTop: "30px" }}>
            <div style={{ fontSize: "28px", marginBottom: "8px" }}>町</div>
            <p>縺ゅ↑縺溘・險隱槭〒繝｡繝・そ繝ｼ繧ｸ繧帝√ｌ縺ｾ縺・/p>
            <p style={{ fontSize: "10px", marginTop: "4px", color: "#bfdbfe" }}>{myLang?.flag} {myLang?.label} 縺ｧ陦ｨ遉ｺ縺輔ｌ縺ｾ縺・/p>
          </div>
        )}
        {messages.map(msg => {
            ...
            <div key={msg.id} style={{ display: "flex", flexDirection: "column", gap: "3px", maxWidth: "85%", alignSelf: isMe ? "flex-end" : "flex-start", alignItems: isMe ? "flex-end" : "flex-start" }}>
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: "5px", flexDirection: isMe ? "row-reverse" : "row" }}>
                ...
                {isMe && features.deleteMessage && (
                  <button onClick={() => deleteMessage(msg.id)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "13px", color: "#d1d5db", padding: "2px", flexShrink: 0, marginTop: "6px" }}>卵・・/button>
                )}
              </div>
              {tline && (
        ...
        <div style={{ display: "flex", gap: "8px", alignItems: "flex-end" }}>
          <div style={{ position: "relative", flex: 1 }}>
            ...
            <textarea ref={textareaRef} value={input}
              ...
              onKeyDown={handleKey}
              placeholder={`${myLang?.label}縺ｧ蜈･蜉・/ Type in ${myLang?.label}`}
              rows={1}
